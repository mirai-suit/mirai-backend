...
//       // not using rn beacuse we are using seperate function for password reset
...
export const login = async (userData: LoginInput) => {
  ...
  const refreshToken = await generateAndSaveRefreshToken(userAuth.id);
  return {
    user: {
      id: userAuth.id,
...
//     if (
//       lastOtpRequest &&
//       new Date().getTime() - lastOtpRequest.requestedAt.getTime() < 60000
//     ) {
